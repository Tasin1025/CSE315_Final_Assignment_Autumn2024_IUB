...
std::mutex mtx;       // Mutex for thread safety on the shared totalArea variable
double totalArea = 0; // Variable to store the estimated area

// Function representing the mathematical function to approximate (π in this case)
double f(double x)
{
    return pow(x, 4) * exp(-x); // f(x) = 4 / (1 + x^2), which is used to approximate π
}
